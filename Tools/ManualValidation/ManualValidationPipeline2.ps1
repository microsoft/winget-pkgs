...
Function Get-TrackerVMRunTracker {
	while ($True) {
			...
			if (!(($status | Where-Object {$_.version -ne (Get-TrackerVMVersion)}).Count)) {
				if (!($status | Where-Object {($_.mode -join $Enum.Char.Space) -match "Creation"})) {
					if ($PRQueueCount -gt 0) {
						if ((Get-ArraySum $status.RAM) -lt ($SystemRAM*$RamPctForVms)) {
							$PR = Get-SchemaCheck -InputData (Get-PopPRQueue) -SchemaInfo $MVSchemaData.PR
							if ($null -ne $PR) {
		...
		# } elseIf ($clip -match $Enum.Strings.WindowsPackageManager) {#Package Manager Dashboard
			#Write-Output "Gathering PR Headings"
		} elseIf ($clip -match $Enum.Regex.manifestsRegex) {#Example: /manifests/t/tritant/MiniMediaEdit/1.0/tritant.MiniMediaEdit.installer.yaml
			$GhRlRemain = ((Get-GitHubRateLimit) | where {$_.source -match $Enum.GitHubRateLimit.Unlogged}).remaining
...
Function Get-PRWatch {
	...
	Param(
		...
		[string[]]$clip = (Get-CleanClipboard -Type "StrArray"),
		...
		foreach ($PR in $Results) {
			...
			if ($PRtitle) {
				if (Compare-Object $PRtitle $oldclip) {
						...
						$Submitter = (($clip | Select-String $Enum.Strings.WantsToMerge) -split $Enum.Char.Space)[$Enum.Index.First]
						...
						try {
							...
							try {
								$prVersion = (Get-YamlValue $Enum.ManifestKeys.PackageVersion -clip $clip) | Get-RemoveQuotes
									...
									try {
								[System.Version]$prVersion = (Get-YamlValue $Enum.ManifestKeys.PackageVersion -clip $clip) #try *with* the quotes
								} catch {
									if ($null -ne $PRVerLoc) {
										try {
					...
					# Write-Host "PR version $prVersion"
						...
						try {
							$PackageIdentifier = Get-SchemaCheck -InputData $clip -MatchWith $Enum.ManifestKeys.PackageIdentifier -SchemaInfo $schemaData.installer.definitions.PackageIdentifier 
							$PackageIdentifier = $PackageIdentifier -replace $Enum.Char.DoubleQuote,$Enum.Char.Blank
						} catch {
						...
						($AgreementAccept -notmatch "[ + ]")) {
							...
							[bool]$WordFilterMatch = $False
							Foreach ($Word in $Enum.WordFilterList) {
								$WordFilterMatch += ($Clip -match $Word) # -notmatch $Enum.Strings.Url -notmatch $Enum.Strings.Agreement
							}
							...
							if ($null -ne $ValToEval) {
								...
								($PRtitle -notmatch $Enum.PRWatch.Remove)) {
									...
									$DeveloperIsAuthor = (((Get-SchemaCheck -InputData $clip -MatchWith $Enum.ManifestKeys.PackageIdentifier -SchemaInfo $schemaData.installer.definitions.PackageIdentifier ) -split ".") -ceq $Submitter)
	...
	Foreach ($Preset in $PresetList) {
		...
		While ($PRs.Length -gt 0) {
			...
			Foreach ($FullPR in $PRs) {
				...
				$PR = Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData $FullPR.number
...
Function Get-GitHubPreset {
	...
	$PR = Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData $PR
	...
	if (($PR.ToString().Length -eq 6) -OR $Force) {
		Switch ($Preset) {
			...
			$Enum.GitHubPresets.PossibleDuplicate {
				...
				$PullInstallerContents = (Get-DecodeGitHubFile ((Invoke-GitHubRequest -Uri $Pull.contents_url[$Enum.Index.First] -JSON).content))
				$Url = (Get-YamlValue -Key InstallerUrl -clip $PullInstallerContents)
				$PackageIdentifier = (Get-SchemaCheck -MatchWith $Enum.ManifestKeys.PackageIdentifier -SchemaInfo $schemaData.installer.definitions.PackageIdentifier -InputData $PullInstallerContents)
...
Function Get-PRLabelAction { #Soothing label action.
	Param(
		...
		$PR2 = (Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData $PR),
...
Function Get-RandomIEDS {
	Param(
		...
		$PR = (Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData ($IEDSPRs.number | where {$_ -notin (Get-Status).pr} | Get-Random))#,
...
Function Get-PRManifest {
	...
	# $n++
	# }
	$PackageIdentifier = Get-SchemaCheck -MatchWith $Enum.ManifestKeys.PackageIdentifier -SchemaInfo $schemaData.installer.definitions.PackageIdentifier -InputData $CommitFile | Get-RemoveQuotes
...
Function Get-FileFromGitHub {
	Param(
		...
		$Path = ($PI -replace "[.]",$Enum.Char.Slash),
		$FirstLetter = ($PI[$Enum.Index.First].tostring().tolower())
	)
...
Function Get-CheckIfPackageIsNew {
	Param(
		[Parameter(ValueFromPipeline)][int]$PR,
		$PR2 = (Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData $PR),
		$prdata = (Get-CommitFile -PR $PR2),
		[string]$PackageIdentifier = (Get-SchemaCheck -MatchWith $Enum.ManifestKeys.PackageIdentifier -SchemaInfo $schemaData.installer.definitions.PackageIdentifier -InputData $PRData),
		$ManifestVersion = (Get-ManifestVersion -PackageIdentifier $PackageIdentifier)
	)
	Process {
		$PR = $PR2
...
function Get-PushMePRYou {
	...
	foreach ($Preset in ($Enum.SearchPresets.Approval,$Enum.SearchPresets.ToWork)) {
		...
		if ($PRsForAuthor) {
			$PRsForAuthor.number | % { 
				$PR = (Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData $_)
...
Function Get-CheckData {
	Param(
		[Parameter(ValueFromPipeline)][int]$PR,
		$PR2 = (Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData $PR),
		$PRData = (Invoke-GitHubRequest "$GitHubApiBaseUrl/pulls/$PR2" -JSON),
...
Function Get-AddToAutowaiver {
	Param(
		...
		$PackageIdentifier = (Get-SchemaCheck -MatchWith $Enum.ManifestKeys.PackageIdentifier -SchemaInfo $schemaData.installer.definitions.PackageIdentifier -InputData $PRData),
...
Function Get-Autowaiver {
	Param(
		[Parameter(ValueFromPipeline)][int]$PR,
		$PR2 = (Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData $PR2),
		$PRData = (Get-CommitFile -PR $PR2),
		$PackageIdentifier = (Get-SchemaCheck -MatchWith $Enum.ManifestKeys.PackageIdentifier -SchemaInfo $schemaData.installer.definitions.PackageIdentifier -InputData $PRData),
		$AutowaiverData = (Get-Content $AutowaiverFile | ConvertFrom-Csv),
...
Function Get-TrackerVMValidate {
	Param(
		[string[]]$clip = ((Get-CleanClipboard -Type "StrArray") -split $Enum.Char.LineBreak),
		...
		[string]$PackageIdentifier = ((Get-SchemaCheck -InputData $clip -MatchWith $Enum.ManifestKeys.PackageIdentifier -SchemaInfo $schemaData.installer.definitions.PackageIdentifier ) | Get-RemoveQuotes),
		[string]$PackageVersion = ((Get-YamlValue -Key $Enum.ManifestKeys.PackageVersion -clip $clip) | Get-RemoveQuotes),
		[int]$PR = (Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData (Get-PRNumber ($clip -replace $Enum.Char.Hash," #") -Hash)),
	...
	if (($PackageIdentifier) -AND ($PackageIdentifier -ne $Enum.ManifestKeys.PackageIdentifier)) {
		...
		($PRState.state -ne $Enum.PRStates.Open)) {
			...
			if ($Operation -eq $Enum.PRTrackerOperations.Configure) {
				...
				if ($notElevated -OR ($clip | Select-String $Enum.Strings.ElevationRequirementElevationProhibited)) {
					if (!($Silent)) {
...
Function Get-ManifestForValidation {
	Param(
		[int]$vm,
		$clip, #= ( -split $Enum.Char.LineBreak), [string[]]]?
		[string]$PackageIdentifier = (Get-SchemaCheck -InputData $clip -MatchWith $Enum.ManifestKeys.PackageIdentifier -SchemaInfo $schemaData.installer.definitions.PackageIdentifier ),
        [ValidateScript( { $_ -in (Get-Keys $Enum.PRTrackerOperations) } )][string]$Operation = $Enum.PRTrackerOperations.Scan,
		[switch]$Silent,
		[switch]$WhatIf,
		$VMFolder = "$MainFolder\vm\$vm",
...
Function Get-ManifestFile {
	Param(
		...
		[string]$PackageIdentifier = ((Get-SchemaCheck -InputData $clip -MatchWith $Enum.ManifestKeys.PackageIdentifier -SchemaInfo $schemaData.installer.definitions.PackageIdentifier ) -replace $Enum.Char.DoubleQuote,$Enum.Char.Blank -replace $Enum.Char.SingleQuote,$Enum.Char.Blank -replace $Enum.Char.Comma,$Enum.Char.Blank),
		[int]$PR,
		[string]$Arch,
		[string]$OS,
		[string]$Scope
...
Function Get-ListingDiff {#Might Depreciate
	Param(
		$Clip,
		$PackageIdentifier = ((Get-SchemaCheck -InputData $clip -MatchWith $Enum.ManifestKeys.PackageIdentifier -SchemaInfo $schemaData.installer.definitions.PackageIdentifier ) -replace $Enum.Char.DoubleQuote,$Enum.Char.Blank),
...
Function Get-CommitFile {
	Param(
		...
		$PR2Data = (Invoke-GitHubRequest "$GitHubApiBaseUrl/pulls/$PR2" -JSON),
...
Function Get-UpdateHashInPR2 {
	Param(
		...
		$PR2 = (Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData $PR),
		$Clip,
		[string]$SearchTerm = "Expected hash",
		[string]$ManifestHash = (Get-YamlValue $SearchTerm -Clip $Clip),
		$LineNumbers = ((Get-CommitFile -PR $PR2 | Select-String $ManifestHash).LinEnumber),
		[string]$ReplaceTerm = "Actual hash",
		[string]$ReplaceString = ("  InstallerSha256: " + (Get-YamlValue $ReplaceTerm -Clip $Clip).toUpper()),
		[string]$comment = "``````suggestion`n$ReplaceString`n```````n`n(Deterministic automation - build $build.)"
	)
	$PR = $PR2
	foreach ($Line in $LineNumbers) {
...
Function Get-PopPRQueue {
	...
	$out = Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData $PRQueue[$Enum.Index.First]
...
Function Get-CleanPRExcludeFile {
	...
	Get-Content $PRExcludeFile | %{
			$PR = (Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData $_)
			$PRData = (Invoke-GitHubRequest "$GitHubApiBaseUrl/pulls/$PR" -JSON); if ($PRData.state -eq $Enum.PRStates.Open){
				$out +=  $PR
			}
		}
...
Function Get-PRPopulateRecord {
	...
	Foreach ($Log in $Logs) {
		...
		$ThisPR = (Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData $_.PR)
		$LogPR = (Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData $Log.PR)
...
Function Get-PRReportFromRecord {
	...
	Foreach ($PR in $Record) {
		$Title = $PR.Title
		$PR = Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData $PR.PR
		if (!($Title)) {
			$Title = (Invoke-GitHubPRRequest -PR $PR -Type $Enum.Char.Blank -Output $Enum.PRRequestOutput.Content -JSON).title
		}
...
Function Get-ManifestCovertReview {
	...
	$PR = Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData ($PRtitle -split $Enum.Char.Hash)[$Enum.Index.Second]
...
Function Get-SchemaCheck {
	Param(
		$InputData,
		# [ValidateSet("array", "integer", "object", "string")][string]$InputType,
		[string]$MatchWith,
		$SchemaInfo,
		[switch]$Debug
	)
	#Type, minLength/maxLength Regex/Enum, Transform
	try {
		#If the schema has this field, perform the check.
		#And cleanse of quotes?
		# - "array", "integer(long)", "object", "string"
		[System.Collections.ArrayList]$SchemaKeys = Get-Keys $SchemaInfo
		if (!($SchemaKeys)){
			if ($debug) {write-host "Schema empty FastFail $SchemaKeys"};
			Return $null
		}
		if ($debug) {write-host "Schema contains: $SchemaKeys"};
		if ($debug) {write-host "Schema description: $($SchemaInfo.description)"};
			$SchemaKeys.Remove("description")

		$InputData = $InputData -replace $Enum.Regex.CleanClipRegex,$null
		if ($debug) {write-host "InputData: $InputData"};
		$DataToVerify = $null


		if ($MatchWith) {#Yaml, Json, etc
			if ($debug) {write-host "MatchWith: $MatchWith"};
			if ($InputData -match $MatchWith) {#Yaml, Json, etc
				[string]$mid = Get-YamlValue -Key $MatchWith -clip $InputData
		...
		if ($SchemaInfo.($Item)) {
			...
			$SchemaStrand = ($SchemaInfo.($Item) | where {$_ -ne "null"})
			if ($SchemaStrand -eq $integer) {
				[int]$mid = $DataToVerify
				$DataStrand = $mid.gettype().name -replace "Int32",$integer
				if ($DataStrand -eq $SchemaStrand) {
					if ($debug) {write-host "Schema $Item Pass"};
				} else {
					if ($debug) {write-host "Schema $Item FastFail Data $DataStrand Schema $SchemaStrand"};
					Return $Null
				}
			} else {
		...
		if ($SchemaInfo.($Item)) {
			...
			$SchemaStrand = $SchemaInfo.($Item) -replace "/","\/"
		...
		if ($SchemaInfo.($Item)) {
			...
			if ($DataStrand -notin $SchemaStrand) {
				if ($debug) {write-host "Schema $Item Pass"};
			} else {
				if ($debug) {write-host "Schema $Item FastFail Data $DataStrand Schema $SchemaStrand"};
				Return $Null
		...
		if ($SchemaKeys) {
			# write-host "Error: Unverified schema keys: $SchemaKeys" -f red
			if ($debug) {write-host "Error: Unverified schema keys: $SchemaKeys" -f red};
		} else {
			return $DataToVerify
		}
		
	} catch {
				if ($debug) {write-host "Schema Error FastFail"};
...
Function Get-QueryClipboard { 
	Param(
		[ValidateSet("AllPRsOnClipboard","TrackerVMRunTracker","PRWatch","TrackerVMValidate")]
		[string]$Query,
	...
	$StringMaxLength = 256
	$StrArrayMaxLength = 256
	$StringRegex = "[a-zA-Z]{0,256}"#Need to add a few more chars.

	if ($StrArray) {
			if ($debug) {write-host "Init StrArray Length FastFail $($StrArray.length)"};
	} else {
		try {
			[string[]]$StrArray = ((Get-Clipboard) -replace $Enum.Regex.CleanClipRegex,$null)
		} catch {
			if ($debug) {write-host "Get-Clipboard type FastFail"};
			Return $Null
		}
	}
...
# $PackageIdentifier = (Get-SchemaCheck -InputData $PackageIdentifier -SchemaInfo $schemaData.installer.definitions.PackageIdentifier),
	...
	Switch ($Query) {
		"PRWatch" {
			$out = "" | Select-Object PRtitle,InstallerType,PackageVersion,PackageIdentifier,AgreementUrl,WordFilter,DisplayVersion,InstallerUrl

			[string]$out.PRtitle = $null
			[string]$out.DisplayVersion = $null
			# [ValidateScript( { $_ -in $schemaData.installer.definitions.InstallerType.enum} )][string]$out.InstallerType = $null
			[string]$out.PackageVersion = $null
			[string]$out.PackageIdentifier = $null
			# [string]$out.InstallerUrl = $null #Not sure if needed.
			[string]$out.AgreementUrl = $null
			[bool]$out.WordFilter = $False
			[bool]$out.InstallerUrl = $False

			if ($StrArray) {
				$out.PRtitle = (Get-QueryClipboard -Query AllPRsOnClipboard -StrArray $StrArray)[0]
			}
			foreach ($String in $StrArray) {
				if ($String.length -gt $StringMaxLength) {if ($debug) {write-host "PRWatch String Length FastFail $($String.length)"};Return $Null}
				# If ($String -notmatch $StringRegex) {if ($debug) {write-host "PRWatch String Regex FastFail"};Return $Null}

				
				$EnumVal = $Enum.ManifestKeys.DisplayVersion
				if ($String -match $EnumVal) {
					# $schemaData.installer.definitions.AppsAndFeaturesEntry.properties.DisplayName.type | where {$_ -ne "null"}
					# If ($String -notmatch $StringRegex) {if ($debug) {write-host "PRWatch String Regex FastFail"};Return $Null}
					# $out.DisplayVersion = Get-YamlValue -Key $Enum.ManifestKeys.DisplayVersion -clip $String
					$out.DisplayVersion = Get-SchemaCheck -InputData $String -MatchWith $EnumVal -SchemaInfo $schemaData.installer.definitions.AppsAndFeaturesEntry.properties.($EnumVal)
				}# end if StrArray

				$EnumVal = $Enum.ManifestKeys.InstallerType
				if ($String -match $EnumVal) {
				# If ($String -notmatch $StringRegex) {if ($debug) {write-host "PRWatch String Regex FastFail"};Return $Null}
					# $out.InstallerType = Get-YamlValue -Key $Enum.ManifestKeys.InstallerType -clip $String
					$out.InstallerType = Get-SchemaCheck -InputData $String -MatchWith $EnumVal -SchemaInfo $schemaData.installer.definitions.($EnumVal)
				}# end if StrArray

				if ($String -match $Enum.ManifestKeys.PackageVersion) {
					# $DataToVerify = Get-YamlValue -Key $Enum.ManifestKeys.PackageVersion -clip $String
					# if ($DataToVerify.length -gt $schemaData.installer.definitions.PackageVersion.maxLength) {if ($debug) {write-host "PRWatch PackageVersion Schema Length FastFail $($String.length)"};Return $Null}
					# if ($DataToVerify -notmatch ($schemaData.installer.definitions.PackageVersion.pattern -replace "/","\/")) {if ($debug) {write-host "PRWatch String Schema Regex FastFail $($String.length)"};Return $Null}
					# $out.PackageVersion = $DataToVerify 
					$out.PackageVersion = Get-SchemaCheck -InputData $String -MatchWith $Enum.ManifestKeys.PackageVersion -SchemaInfo $schemaData.installer.definitions.PackageVersion
					
				}# end if StrArray

				if ($String -match $Enum.ManifestKeys.PackageIdentifier) {
					$out.PackageIdentifier = Get-SchemaCheck -InputData $String -MatchWith $Enum.ManifestKeys.PackageIdentifier -SchemaInfo $schemaData.installer.definitions.PackageIdentifier 
				}# end if StrArray

				if ($String -match $Enum.ManifestKeys.AgreementUrl) {
					$out.AgreementUrl = Get-SchemaCheck -InputData $String -MatchWith $Enum.ManifestKeys.AgreementUrl -SchemaInfo $schemaData.locale.definitions.Url
					# $out.AgreementUrl = Get-YamlValue -Key $Enum.ManifestKeys.AgreementUrl -clip $String
				}# end if StrArray
			}# end foreach String
			
			foreach ($Word in $Enum.WordFilterList) {
				foreach ($String in $StrArray) {
					If ($String -notmatch $StringRegex) {if ($debug) {write-host "WordFilterList String Regex FastFail"};Return $Null}
					if ($String -match $Word) {
						[bool]$out.WordFilter += $True
					} else {
						[bool]$out.WordFilter += $False
					}# end if string
				}# end foreach String
			...
			foreach ($String in $StrArray) {
				If ($String -notmatch $StringRegex) {if ($debug) {write-host "InstallerUrl String Regex FastFail"};Return $Null}
				if ($String -match $Enum.ManifestKeys.InstallerUrl) {
					[bool]$out.InstallerUrl += $True
				} else {
					[bool]$out.InstallerUrl += $False
				}# end if StrArray
			}# end foreach String
		...
		"AllPRsOnClipboard" {
			...
			Foreach ($String in $StrArray) {
				...
				if ($debug) {write-host "AllPRsOnClipboard String $String"};
				[string[]]$SplitString = $String -replace "#"," " -split $Enum.Char.Space
				Foreach ($SubString in $SplitString) {
					if ($debug) {write-host "AllPRsOnClipboard SubString $SubString"};
					[int]$mid = (Get-SchemaCheck -SchemaInfo $MVSchemaData.PR -InputData $SubString)
					if (($mid -gt 9999) -AND ($mid -lt 1000000)) {
						if ($debug) {write-host "AllPRsOnClipboard mid $mid out $out"};
						$out += $mid
					}
				}
			}
			$out = $out | Select-Object -Unique
			if ($debug) {write-host "AllPRsOnClipboard out $out"};
			Return $out
		}
		"TrackerVMRunTracker" {
<#
- Matches string return bool.
  -match $ADOMSBaseUrl
  -match "Skip to content"
  -match " Windows Package Manager"
  -match "^manifests`/"
#>
		...
		"TrackerVMValidate" {
<#
- Matches regex return string. (And cleanse of quotes?)
  [int]$PR = (Get-PRNumber ($clip -replace $Enum.Char.Hash," #") -Hash),
- Matches string return bool.
  | Select-String "ElevationRequirement: elevationProhibited"
- Matches YamlKey return strings. (And cleanse of quotes?)
  | Select-String -notmatch "arm" | Select-String "Architecture: " )|ForEach-Object{($_ -split ": ")[$Enum.Index.Second]} 
  $Enum.ManifestKeys.PackageIdentifier | Get-RemoveQuotes
  $Enum.ManifestKeys.PackageVersion | Get-RemoveQuotes
#>			
		}
...
Function Get-CleanClipboard { 
	Param(
		[Parameter(ValueFromPipeline)]$clip = ((Get-Clipboard) -replace $Enum.Regex.CleanClipRegex,$null),
		[ValidateSet("Array","Bool","BoolArray","Hashtable","HashArray","Int","IntArray","String","StrArray")][string]$Type,
		[ValidateSet("Array","Bool","BoolArray","Hashtable","HashArray","Int","IntArray","String","StrArray")][string]$Subtype,
		[ValidateSet("http","json","PRs","yaml")][string]$Verify,
		[ValidateSet("installer","locale","defaultLocale","version")][string]$Schema,
		[string]$Key,
		[switch]$JSON,
		[switch]$Display
	)
	$schemaInUse = $schemaData.($Schema).definitions
	If ($Key) {
		$schemaKeys = Get-Keys $schemaInUse
		if ($JSON) {
			$Key = "`"$($Key)`": "
		} else {
			$Key = "$($Key): "
		}
		if ($Clip -notmatch $Key) {return}
	}
	
	
	Switch ($Verify) {
		"http" {
			if ($Type -notmatch "String") {return}
			if ($Clip -notmatch "http") {return}
		}
		{($_ -eq "json") -or ($_ -eq "yaml")} {
			# if ($Type -notmatch "String") {return}
		...
		"PRs" {
			if ($Type -notmatch "Array") {return}
			if ($Subtype -notmatch "Int") {return}
...
Function Get-YamlValue {
	Param(
		[string]$Key,
		[Parameter(ValueFromPipeline)]$clip,
		[switch]$JSON,
		[switch]$Display
	)
	Process {
...
Function Get-AllPRsOnClipboard {
	return Get-QueryClipboard -Query AllPRsOnClipboard
}
